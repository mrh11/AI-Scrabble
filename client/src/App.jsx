...
const App = () => {
  ...
  const [playerState, changePlayer] = useState(1);
  ...
  const [p2Rack, updatePlayer2Rack] = useState([]);
  const [word, changeWord] = useState('');
  const [currentLetter, setLetter] = useState('')
  const [currentBoardTile, setCurrentBoardTile] = useState('');
  ...
  <div >
    ...
    <button onClick={()=>{(playerState === 1) ? changePlayer(2) : changePlayer(1);}} className="button">Switch Player</button>
  <div >
    <BoardView grid={grid} updateGrid={updateGrid} setLetter={setLetter} currentLetter={currentLetter} currentBoardTile={currentBoardTile} setCurrentBoardTile={setCurrentBoardTile}/>
  </div>
